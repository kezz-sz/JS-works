...
function chatBotResponse(userMessage) {
    ...
    $(document).ready(function autoScroll(){
        $('#chat').animate({ scrollTop: 300000000000 }, 5);
    })

    const userMessageSemAcentos = removerAcentos(userMessage).toLowerCase();
    if (userMessageSemAcentos.includes("nao") && userMessageSemAcentos.includes("comprar")) {
        responseMessage.innerHTML = `<div class="message">Já tentou verificar sua internet? Ou se você tem algum tipo de VPN?</div>`;
    }
